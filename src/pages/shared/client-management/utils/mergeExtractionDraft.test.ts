...
describe("mergeExtractionDraft", () => {
  it("fails closed when overwrite imports a changed primary address over an attested address", () => {
    ...
    Object.assign(initial.stage1, { address: "42 Service Lane", location: { lat: "40.7", lon: "-74" }, line1: "42 Service Lane", city: "Newark", state: "NJ", postalCode: "07102", country: "US", payrollServiceLocations: [{ source: "primaryAddress" as const, attestedActualServiceLocation: true as const, effectiveFrom: "2026-08-14" }] });
    ...
    expect(formData.stage1.payrollServiceLocations).toEqual([]);
    expect(formData.stage1.line1).toBeUndefined();
  });

  it("keeps an absent payroll service-location choice absent when an import changes the primary address", () => {
    const initial = createInitialAddClientFormData();
    Object.assign(initial.stage1, { address: "42 Service Lane", location: { lat: "40.7", lon: "-74" }, line1: "42 Service Lane", city: "Newark", state: "NJ", postalCode: "07102", country: "US", payrollServiceLocations: undefined });

    const { formData } = mergeExtractionDraft(initial, makeExtraction({ draft: { stage1: { address: "99 New Street" } } }), { overwrite: true });

    expect(formData.stage1.payrollServiceLocations).toBeUndefined();
  });

  it("mirrors an imported HHA unit into line2 and clears an attestation", () => {
    const initial = createInitialAddClientFormData();
    initial.type = "hha";
    Object.assign(initial.stage1, { line1: "42 Service Lane", line2: "Unit 3", city: "Newark", state: "NJ", postalCode: "07102", country: "US", homeInfo: { ...initial.stage1.homeInfo, apartmentNumber: "Unit 3" }, payrollServiceLocations: [{ source: "primaryAddress" as const, attestedActualServiceLocation: true as const, effectiveFrom: "2026-08-14" }] });
  ...
  it("fails closed for a non-overwrite import that fills blank legacy address and unit fields", () => {
    const initial = createInitialAddClientFormData();
    initial.type = "hha";
    Object.assign(initial.stage1, { line1: "42 Service Lane", line2: "Unit 3", city: "Newark", state: "NJ", postalCode: "07102", country: "US", payrollServiceLocations: [{ source: "primaryAddress" as const, attestedActualServiceLocation: true as const, effectiveFrom: "2026-08-14" }] });
  ...
  it("invalidates an attestation when an import keeps the address but changes its zip", () => {
    const initial = createInitialAddClientFormData();
    Object.assign(initial.stage1, { address: "42 Service Lane", zipCode: "07102", location: { lat: "40", lon: "-74" }, line1: "42 Service Lane", city: "Newark", state: "NJ", postalCode: "07102", country: "US", payrollServiceLocations: [{ source: "primaryAddress" as const, attestedActualServiceLocation: true as const, effectiveFrom: "2026-08-14" }] });
  ...
  it("clears an old HHA unit when a changed imported address supplies no unit", () => {
    const initial = createInitialAddClientFormData();
    initial.type = "hha";
    Object.assign(initial.stage1, { address: "42 Service Lane", line2: "Unit 3", homeInfo: { ...initial.stage1.homeInfo, apartmentNumber: "Unit 3" }, payrollServiceLocations: [{ source: "primaryAddress" as const, attestedActualServiceLocation: true as const, effectiveFrom: "2026-08-14" }] });
    const { formData } = mergeExtractionDraft(initial, makeExtraction({ draft: { stage1: { address: "99 New Street" } } }), { overwrite: true });
