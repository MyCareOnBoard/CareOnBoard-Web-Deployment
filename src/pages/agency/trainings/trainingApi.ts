...
export interface TrainingStaff { id: string; fullName: string; profilePictureUrl: string; status: string; assignedCount: number; requiredCount?: number; }
export interface TrainingStaffPage { items: TrainingStaff[]; nextCursor: string | null; }
export interface TrainingSummary { assigned: number; manualCompleted: number; policyAwaitingReview: number; policyAccepted: number; policyAssessmentComplete?: boolean; policyPendingAssignmentCount?: number; }
...
export const employeeTrainingsApi = createApi({
    ...
    endpoints: builder => ({
        ...
        policyEvidence: builder.mutation<unknown, {trainingId: string; employeeId: string; action: 'review' | 'revoke-evidence'; data: Record<string, unknown>}>({
