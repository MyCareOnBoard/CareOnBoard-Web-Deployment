...
interface AgencySupportedEmploymentInterventionProps {
...
export default function AgencySupportedEmploymentIntervention(
  ...
  useEffect(() => {
    if (!isLoading && submittedNote && submittedNote.notes.length > 0) {
      ...
      const interventionNotes = submittedNote.notes.filter((note) => note.metadata?.type === "intervention");
    ...
    <VoiceRecordingProvider pageTitle={pageTitle}>
      <div className="min-h-[calc(100vh-200px)] pb-20">
        ...
        <h2
          ...
          Supported Employment Services – Intervention Plan and Service Log ({submittedNote?.metadata?.serviceCode})
        </h2>
        ...
        <div className="overflow-x-auto mb-6">
          <div className="w-full min-w-[1163px]">
            ...
            <div className="border border-[#b2b2b3] rounded-bl-[2px] rounded-br-[2px] border-t-0 overflow-hidden">
              <table className={`w-full ${isEditable ? "bg-[#eef4f5]" : "bg-white"}`} style={{borderCollapse: 'collapse'}}>
                <tbody>
                        ...
                        <tr className="flex flex-col min-h-[147px]">
                          ...
                          <td>
                            ...
                            {isEditable ? (
                              ...
                              <span className="text-[14px] font-normal leading-[1.4] text-[#10141a] font-['Urbanist',sans-serif] ps-2">
                                {service.datesOfSeServices.seProfessional}
                              </span>
                            )}
                          </td>
                        </tr>
                        ...
                        {Object.entries(service.noOfHours).map(([key, value]) => (
                          <tr key={key} className="flex flex-col h-[49px]">
                            <td className="border-b border-[#b2b2b3] h-[49px] flex">
                              <div className="bg-[#D9D9D9] w-[80px] h-full flex items-center justify-end pe-5 capitalize">
                                {key}:
                              </div>
                              <div className="flex-1 flex items-center justify-center">
                                {isEditable && (key === 'start' || key === 'end') ? (
                                  <TimePicker
                                    value={value}
