...
import { getDayPrograms, type DayProgram, type Attendee } from "@/lib/api/day-programs";
...
export default function DayProgramHistoryPage() {
    const navigate = useNavigate();
    const { user } = useAuth();
    const { toast } = useToast();

    const [selectedDate, setSelectedDate] = useState<Date | null>(new Date());
    const [programs, setPrograms] = useState<DayProgram[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const response = await getDayPrograms({
                    date: dateStr,
                    limit: 100
                });
                ...
                toast({
                    title: "Error",
                    description: "Failed to load day program history.",
                    variant: "destructive"
                });
            } finally {
                setIsLoading(false);
    ...
    const tableData = useMemo(() => {
        ...
        programs.forEach(prog => {
            if (prog.attendees && Array.isArray(prog.attendees)) {
                prog.attendees.forEach((attendee: Attendee) => {
                    rows.push({
                        id: attendee.id || `${prog.id}-${Math.random()}`,
                        submissionId: prog.id,
                        name: attendee.name,
