...
it('keeps no-expiry informational and filters only displayed client rows without changing the page',()=>{
  ...
  expect(screen.getByText('On file — expiry not recorded')).toBeInTheDocument();fireEvent.click(screen.getByRole('checkbox',{name:'Needs attention on this page'}));expect(screen.queryByText('Info')).not.toBeInTheDocument();expect(screen.getByText(/Review/)).toBeInTheDocument();expect(screen.getByText(/Checklist unavailable. Saved/)).toBeInTheDocument();expect(props.onPage).not.toHaveBeenCalled();expect(props.onApply).not.toHaveBeenCalled();expect(screen.getByRole('button',{name:'Next page'})).toBeEnabled();
});
