...
describe('workspace access and URL boundary', () => {
  it('intersects workspace with independent source grants', () => {
    ...
    expect(getComplianceSources(staff(['Compliance Alerts', 'Client Management']), 'hha')).toEqual(['client_documents', 'unsigned_form485']);
  ...
  it('retains owner and SC staff capabilities without broadening explicit grants', () => {
    expect(getComplianceSources({userType: 'agency', agency: {supportedClientTypes: ['ddd']}}, 'ddd')).toEqual(['document_expiry', 'training', 'client_documents', 'shift_notes']);
