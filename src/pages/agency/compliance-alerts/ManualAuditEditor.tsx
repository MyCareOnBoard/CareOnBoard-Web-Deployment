...
export default function ManualAuditEditor(p:ManualAuditEditorProps) {
  ...
  if(stateScope!==scope)return <section className="p-4 sm:p-6"><p role="status">Loading review…</p></section>;
  ...
  if(!checklist||!draft)return <section className="p-4 sm:p-6">{detail.error?<><p role="alert">{errorText(detail.error)}</p><Button variant="outline" onClick={()=>void detail.refetch()}>Retry loading</Button></>:<p role="status">Loading review…</p>}<Button variant="outline" onClick={p.onClose}>Close</Button></section>;
