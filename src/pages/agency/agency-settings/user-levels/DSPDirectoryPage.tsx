...
import { useGetDSPsReportQuery } from "@/lib/api/reports";
...
export default function DSPDirectoryPage() {
    ...
    const { data, isLoading } = useGetDSPsReportQuery({ isLifetime: true });

    const users = useMemo(() => {
        if (!data?.data) return [];
        return data.data.map((dsp) => ({
            id: dsp.id,
            name: dsp.fullName,
            avatar: dsp.profilePictureUrl ?? undefined,
            status: (dsp.status === "active" ? "Active" : "Inactive") as "Active" | "Inactive",
            clients: dsp.clientCount ?? 0,
            trainingCompleted: dsp.trainingCompleted ?? 0,
            trainingTotal: dsp.trainingTotal ?? 0,
        }));
    }, [data]);
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col gap-4 shadow-sm p-6 bg-white rounded-[16px] mb-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-6 sm:gap-8">
                    <div className="text-right">
                        <div className="text-[32px] font-bold text-[#10141a]">{isLoading ? "—" : activeCount}</div>
                    ...
                    <div className="text-right">
                        <div className="text-[32px] font-bold text-[#10141a]">{isLoading ? "—" : inactiveCount}</div>
                        <div className="flex items-center justify-end gap-1.5 mt-1">
                    ...
                    <div className="text-right">
                        <div className="text-[32px] font-bold text-[#10141a]">{isLoading ? "—" : users.length}</div>
                        <div className="flex items-center justify-end gap-1.5 mt-1">
            ...
            <div className="shadow-sm p-6 space-y-5 bg-white rounded-[16px]">
                ...
                <div className="space-y-3">
                    {isLoading ? (
                        <p className="text-center text-sm text-[#808081] py-8">Loading...</p>
                    ) : filteredUsers.length === 0 ? (
                        <p className="text-center text-sm text-[#808081] py-8">No DSPs found.</p>
                    ) : (
