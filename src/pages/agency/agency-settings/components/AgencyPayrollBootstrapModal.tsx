...
type Props = {
  ...
  requireSignerConfirmation?: boolean;
...
function formValues(values: CheckPayrollProfileRead): CheckPayrollProfileFormValues {
  return {
    ...
    payFrequency: values.payrollIntent?.frequency ?? "",
    firstPayday: values.payrollIntent?.firstPayday ?? "",
...
export const AGENCY_PAYROLL_REQUIRED_FIELD_MAP: Record<RequiredFieldCode, FieldSpec> = {
  ...
  "payrollIntent.frequency": { group: "schedule", target: "payFrequency", satisfied: (f) => CHECK_PAY_FREQUENCIES.includes(f.payFrequency as typeof CHECK_PAY_FREQUENCIES[number]) },
  "payrollIntent.firstPayday": { group: "schedule", target: "firstPayday", satisfied: (f) => present(f.firstPayday) },
...
const COMPANY_ERROR_TARGETS: Readonly<Record<string, FormErrorTarget>> = {
  ...
  payrollFirstPayday: "firstPayday",
};
...
export default function AgencyPayrollBootstrapModal({ open, values, missingFieldCodes, isSubmitting = false, submissionError = null, submissionFieldCodes = [], scope, ownerCandidate = null, requireSignerConfirmation = false, onOpenChange, onSubmit }: Props) {
  ...
  const visibleDateError = form.firstPayday ? validationErrors.firstPayday : fieldErrors.firstPayday;
  ...
  return <Dialog open={open} onOpenChange={(nextOpen) => { if (!isSubmitting) onOpenChange(nextOpen); }}>
    <DialogContent showCloseButton={false} className="flex max-h-[90vh] w-[min(94vw,760px)] flex-col overflow-hidden rounded-[30px] border border-[rgba(255,255,255,0.3)] bg-white p-0 backdrop-blur">
      ...
      <form onSubmit={(event) => void submit(event)} className="flex min-h-0 flex-1 flex-col overflow-hidden"><div tabIndex={0} role="region" aria-label="Payroll setup form" className="min-h-0 flex-1 overflow-y-auto px-5 py-5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#00b4b8]/30 sm:px-6"><fieldset disabled={isSubmitting} className="space-y-5 disabled:opacity-70">
        ...
        <DialogFooter className="shrink-0 border-t border-[#e2e8e8] bg-[#fbfcfc] px-5 py-4 sm:px-6"><div className="space-y-3">{(hasValidationErrors || (requireSignerConfirmation && !signerSelection)) && <p id="payroll-bootstrap-validation-help" className="text-xs text-[#5d626b]">{requireSignerConfirmation && !signerSelection ? "Choose an authorized signer and confirm authority to continue." : "Complete all required fields with valid information to continue."}</p>}<div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" disabled={isSubmitting} className="min-h-11 rounded-[10px] border border-[#b2b2b3] px-4 text-sm font-semibold text-[#353535] transition-colors hover:bg-[#f2f4f4] disabled:cursor-not-allowed disabled:opacity-60" onClick={() => onOpenChange(false)}>Cancel</button><button type="submit" disabled={isSubmitting || hasValidationErrors || (requireSignerConfirmation && !signerSelection)} aria-busy={isSubmitting} aria-describedby={hasValidationErrors || (requireSignerConfirmation && !signerSelection) ? "payroll-bootstrap-validation-help" : undefined} className="inline-flex min-h-11 min-w-full items-center justify-center rounded-[10px] bg-[#006f73] px-4 text-sm font-semibold text-white transition-colors hover:bg-[#005f63] disabled:cursor-not-allowed disabled:opacity-60 sm:min-w-[14rem]">{isSubmitting ? <span role="status" className="inline-flex items-center gap-2"><Loader2 data-testid="agency-payroll-modal-create-spinner" aria-hidden="true" className="h-4 w-4 shrink-0 motion-safe:animate-spin" />Creating payroll setup…</span> : "Create payroll setup"}</button></div></div></DialogFooter>
      </form>
    </DialogContent>
  </Dialog>;
}
