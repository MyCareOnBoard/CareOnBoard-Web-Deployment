...
export default function AgencyPayrollBootstrapModal({ open, values, missingFieldCodes, isSubmitting = false, submissionError = null, submissionFieldCodes = [], onOpenChange, onSubmit }: Props) {
  ...
  return <Dialog open={open} onOpenChange={(nextOpen) => { if (!isSubmitting) onOpenChange(nextOpen); }}>
    <DialogContent showCloseButton={false} className="flex max-h-[90vh] w-[min(94vw,760px)] flex-col overflow-hidden rounded-[30px] border border-[rgba(255,255,255,0.3)] bg-white p-0 backdrop-blur">
      ...
      <form onSubmit={(event) => void submit(event)} className="flex min-h-0 flex-1 flex-col"><fieldset disabled={isSubmitting} className="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5 disabled:opacity-70 sm:px-6">
        ...
        {groups.has("schedule") && <div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><EnumField label="Pay frequency" value={form.payFrequency} options={CHECK_PAY_FREQUENCIES} inputRef={firstGroup === "schedule" ? firstSelectRef : undefined} error={fieldErrors.payFrequency} onChange={(payFrequency) => update("payFrequency", payFrequency)} /><PayrollDateField label="First payday" value={form.firstPayday} error={fieldErrors.firstPayday} onChange={(firstPayday) => update("firstPayday", firstPayday)} />{form.payFrequency === "semimonthly" && <PayrollDateField label="Second payday" value={form.secondPayday} error={fieldErrors.secondPayday} onChange={(secondPayday) => update("secondPayday", secondPayday)} />}<PayrollDateField label="First period end" value={form.firstPeriodEnd} error={fieldErrors.firstPeriodEnd} onChange={(firstPeriodEnd) => update("firstPeriodEnd", firstPeriodEnd)} /><PayrollDateField label="Local payroll start date" value={form.payrollStartDate} error={fieldErrors.payrollStartDate} onChange={(payrollStartDate) => update("payrollStartDate", payrollStartDate)} /></div>}
        ...
        <DialogFooter className="shrink-0 border-t border-[#e2e8e8] bg-[#fbfcfc] px-5 py-4 sm:px-6"><div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" disabled={isSubmitting} className="min-h-11 rounded-[10px] border border-[#b2b2b3] px-4 text-sm font-semibold text-[#353535] transition-colors hover:bg-[#f2f4f4] disabled:cursor-not-allowed disabled:opacity-60" onClick={() => onOpenChange(false)}>Cancel</button><button type="submit" disabled={isSubmitting} aria-busy={isSubmitting} className="inline-flex min-h-11 min-w-full items-center justify-center rounded-[10px] bg-[#006f73] px-4 text-sm font-semibold text-white transition-colors hover:bg-[#005f63] disabled:cursor-not-allowed disabled:opacity-60 sm:min-w-[14rem]">{isSubmitting ? <span role="status" className="inline-flex items-center gap-2"><Loader2 data-testid="agency-payroll-modal-create-spinner" aria-hidden="true" className="h-4 w-4 shrink-0 motion-safe:animate-spin" />Creating payroll setup…</span> : "Create payroll setup"}</button></div></DialogFooter>
      </form>
    </DialogContent>
  </Dialog>;
}
