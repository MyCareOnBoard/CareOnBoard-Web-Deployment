...
import { getStaffTimesheet, getStaffTimesheetErrorMessage, reviewStaffTimesheet, type StaffTimesheet } from "@/lib/api/staff-timesheets";
...
function rowToTimesheet(row: NetworkBillingTimesheetRow, startDate: string, endDate: string): AgencyTimesheet {
  return {
    ...
    periodStart: iso(row.periodStart, startDate), periodEnd: iso(row.periodEnd, endDate), entries: [], totalHours: row.totalHours,
    signature: null, signatureInfo: "Signature is loaded only when this timesheet is opened.", status: row.status,
    reviewedAt: null, reviewedBy: null, reviewerNotes: null, payrollInvoiceId: null,
    createdAt: iso(row.createdAt, startDate), updatedAt: iso(row.createdAt, endDate),
  };
}
...
export default function NetworkTimesheets() {
  ...
  useEffect(() => {
    // Selection changes replace the dataset; clear any row-bound review action first.
    setViewing(null); setRejectTarget(null); setRejectNotes("");
  }, [staff]);
  ...
  const invalidate = (agencyId: string) => dispatch(networkBillingApi.util.invalidateTags([{ type: "Timesheets", id: "NETWORK" }, { type: "NETWORK", id: agencyId }]));
  ...
  return <section aria-label="Network timesheets" aria-busy={initial.isLoading || page.isFetching} className="min-w-0 space-y-6 pb-8">
    <div className="flex flex-col gap-2 sm:flex-row sm:justify-end"><label className="sr-only" htmlFor="network-timesheet-staff">Find a staff member</label><input id="network-timesheet-staff" value={staffSearch} onChange={(event) => setStaffSearch(event.target.value)} placeholder="Search staff across agencies" className="min-h-11 w-full rounded-md border border-[#e5e5e6] bg-white px-3 text-sm sm:max-w-xs" />{staff ? <Button type="button" variant="outline" className="min-h-11" onClick={() => { setViewing(null); setRejectTarget(null); setStaff(null); setStaffSearch(""); }}>Clear staff</Button> : null}</div>
    {options.data?.length ? <div role="listbox" aria-label="Authorized staff" className="rounded-xl border border-[#e5e5e6] bg-white p-2">{options.data.map((option) => <button key={`${option.agencyId}:${option.id}`} type="button" role="option" className="block min-h-11 w-full rounded-lg px-3 text-left text-sm hover:bg-[#eef4f5]" onClick={() => { setViewing(null); setRejectTarget(null); setStaff(option); setStaffSearch(option.name); }}>{option.name} <span className="text-[#687173]">· {option.agencyName}</span></button>)}</div> : null}
    <StaffTimesheetsTable timesheets={timesheets} loading={initial.isLoading} isRefetching={page.isFetching} nextCursor={cursor} onLoadMore={() => void loadMore()} showAgency busyId={busyId} onView={(row) => void openTimesheet(row)} onApprove={(row) => void approve(row)} onReject={(row) => { setRejectTarget(row); setRejectNotes(""); }} />
    {loadMoreError ? <p role="alert" className="text-sm text-[#b42318]">{loadMoreError}</p> : null}
    <Dialog open={Boolean(viewing)} onOpenChange={(open) => !open && setViewing(null)}><DialogContent className="flex w-[560px] max-w-[95vw] flex-col gap-4 rounded-[24px] bg-white p-6">{viewing?.loading ? <DialogDescription>Loading the selected agency timesheet...</DialogDescription> : viewing?.error ? <DialogDescription>{viewing.error}</DialogDescription> : detail ? <><DialogHeader><DialogTitle className="pr-6 text-[20px] font-bold leading-snug text-[#10141a]">{detail.staffName || "Staff timesheet"}</DialogTitle><DialogDescription>{viewing!.row.agencyName} · {detail.periodStart} – {detail.periodEnd}</DialogDescription><div className="mt-2 flex flex-wrap items-center gap-2"><StaffTimesheetStatusPill status={detail.status} /><span className="text-[13px] text-[#6b7280]">{detail.periodStart} – {detail.periodEnd}</span></div></DialogHeader><div className="grid grid-cols-3 gap-x-6 gap-y-3"><div><p className="mb-0.5 text-[12px] font-semibold uppercase tracking-wide text-[#808081]">Role</p><p className="text-[14px] text-[#6b7280]">{detail.role || "—"}</p></div><div><p className="mb-0.5 text-[12px] font-semibold uppercase tracking-wide text-[#808081]">Total hours</p><p className="text-[14px] font-semibold text-[#10141a]">{detail.totalHours}h</p></div><div><p className="mb-0.5 text-[12px] font-semibold uppercase tracking-wide text-[#808081]">Program</p><p className="text-[14px] uppercase text-[#6b7280]">{detail.mode}</p></div></div>{detail.reviewerNotes ? <div className="rounded-lg bg-[#fef2f2] p-3"><p className="mb-0.5 text-[12px] font-semibold uppercase tracking-wide text-[#ef4444]">Review notes</p><p className="text-[13px] text-[#7f1d1d]">{detail.reviewerNotes}</p></div> : null}<div><p className="mb-2 text-[12px] font-semibold uppercase tracking-wide text-[#808081]">Entries</p><div className="overflow-hidden rounded-lg border border-[#e5e7eb]"><div className="grid grid-cols-[1fr_1fr_1fr_1fr_60px] gap-2 bg-[#f9fafb] px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-[#808081]"><span>Day</span><span>Date</span><span>In</span><span>Out</span><span>Hrs</span></div><div className="max-h-56 overflow-y-auto">{detail.entries.map((entry, index) => <div key={`${entry.week}-${entry.day}-${index}`} className="grid grid-cols-[1fr_1fr_1fr_1fr_60px] gap-2 border-t border-[#e5e7eb] px-3 py-2 text-[13px] text-[#353535]"><span>{entry.day}</span><span>{entry.date}</span><span>{entry.checkIn}</span><span>{entry.checkOut}</span><span>{entry.hours ?? "—"}</span></div>)}</div></div></div><div><p className="mb-2 text-[12px] font-semibold uppercase tracking-wide text-[#808081]">Signature</p><div className="flex min-h-[60px] items-center rounded-lg border border-[#e5e7eb] px-3 py-2 text-[13px] text-[#6b7280]">{detail.signature ? "Signature received. Its content is redacted in network review." : "Not signed"}</div></div><DialogFooter><Button variant="outline" onClick={() => setViewing(null)}>Close</Button>{detail.status === "pending" ? <><Button variant="outline" onClick={() => { setRejectTarget({ ...viewing!.row, ...detail, agencyName: viewing!.row.agencyName }); setRejectNotes(""); setViewing(null); }}>Reject</Button><Button onClick={() => void approve({ ...viewing!.row, ...detail, agencyName: viewing!.row.agencyName })}>Approve</Button></> : null}</DialogFooter></> : null}</DialogContent></Dialog>
