...
const timesheets = vi.hoisted(() => ({ getStaffTimesheet: vi.fn(), reviewStaffTimesheet: vi.fn(), getStaffTimesheetErrorMessage: vi.fn(() => "Failed") }));
...
vi.mock("@/pages/agency/billing/staff-timesheets/StaffTimesheetsTable", () => ({ StaffTimesheetStatusPill: ({ status }: { status: string }) => <span>{status}</span>, default: ({ timesheets: rows, onView, onReject, onApprove }: { timesheets: Array<{ id: string; staffName: string; totalHours: number }>; onView: (row: unknown) => void; onReject: (row: unknown) => void; onApprove: (row: unknown) => void }) => <div>{rows.map((row) => <div key={row.id}><span>{row.staffName}</span><span>{row.totalHours} hours</span><button onClick={() => onView(row)}>View</button><button onClick={() => onApprove(row)}>Approve</button><button onClick={() => onReject(row)}>Reject</button></div>)}</div> }));
...
const row = { id: "sheet-1", agencyId: "atlas", agencyName: "Atlas Care", staffKey: "private-key", staffUid: "staff-1", staffName: "Avery Nurse", status: "pending" as const, mode: "ddd" as const, periodStart: "2026-07-01", periodEnd: "2026-07-07", totalHours: 8 };
...
describe("NetworkTimesheets", () => {
  ...
  it("binds approval and rejection to the selected row agency", async () => {
    api.options.mockReturnValue({ data: [] }); api.search.mockReturnValue({ unwrap: vi.fn().mockResolvedValue([]), abort: vi.fn() });
    api.initial.mockReturnValue({ data: { page: { rows: [row], nextCursor: null, total: 1, hasMore: false } }, isLoading: false, isFetching: false });
    ...
    expect(api.invalidate).toHaveBeenCalledWith(expect.arrayContaining([{ type: "Timesheets", id: "NETWORK" }, { type: "NETWORK", id: "atlas" }]));
  });
});
