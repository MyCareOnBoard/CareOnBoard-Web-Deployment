...
vi.mock("@/pages/agency/billing/staff-timesheets/StaffTimesheetsTable", () => ({ StaffTimesheetStatusPill: ({ status }: { status: string }) => <span>{status}</span>, default: ({ timesheets: rows, onView, onReject }: { timesheets: Array<{ id: string; staffName: string }>; onView: (row: any) => void; onReject: (row: any) => void }) => <div>{rows.map((row) => <div key={row.id}><span>{row.staffName}</span><button onClick={() => onView(row)}>View</button><button onClick={() => onReject(row)}>Reject</button></div>)}</div> }));
