...
export function validateCompanySetup(values: CheckPayrollProfileFormValues): Record<string, string> {
  ...
  if (hasAddressContent(values.officeAddress) || values.officeName?.trim() || values.actualWorkLocationAttested) {
    if (!values.officeName?.trim() || !address(values.officeAddress) || values.actualWorkLocationAttested !== true) errors.payrollOfficeAddress = "Provide an attested actual workplace address.";
  }
  ...
  if (values.payFrequency) {
    ...
    if (!isoDate(values.firstPayday) || !isoDate(values.firstPeriodEnd) || !isoDate(values.payrollStartDate)) errors.payrollFirstPayday = "Use valid ISO dates.";
    else if (values.firstPeriodEnd! >= values.firstPayday! || values.payrollStartDate! > values.firstPeriodEnd!) errors.payrollFirstPeriodEnd = "The first period must end before payday and after the payroll start date.";
