...
const statusClass: Record<PayStatementStatus, string> = {
  processing: "border-[#2b82ff] text-[#2b82ff]",
  paid: "border-[#0eaf52] text-[#0eaf52]",
  needs_attention: "border-[#ef4444] text-[#ef4444]",
};
...
export function MyPayrollPage() {
  ...
  const scope: EmployeePayrollScope | null = employmentId && user?.uid && user.agencyId ? { audience: "employee", actorUid: user.uid, agencyId: user.agencyId, employmentId } : null;
  ...
  const [loadMore] = useLazyGetEmployeePayStatementsQuery();
  ...
  return <div className="min-h-[calc(100vh-200px)] px-4 sm:px-6 lg:px-0">
    <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      ...
      <label className="flex items-center gap-2 text-sm font-medium text-[#10141a]"><CalendarDays className="h-4 w-4 text-[#6b7280]" /><span>Year</span><select aria-label="Year" value={selectedYear} onChange={(event) => setSelectedYear(Number(event.target.value))} className="h-10 rounded-full border border-[#e5e7eb] bg-white px-3 text-sm outline-none focus:border-[#00b4b8]">{years.map((year) => <option key={year} value={year}>{year}</option>)}</select></label>
    </div>
    {isInitialLoading ? <SummarySkeletons /> : currentData?.setupRequired ? <div className="rounded-2xl border border-[#a7c9ca] bg-[#f7fbfb] p-5"><h2 className="font-semibold text-[#10141a]">Complete payroll setup</h2><p className="mt-1 text-sm text-[#5d626b]">Finish your personal payroll setup before pay statements become available.</p><Link to={setupHref} className="mt-4 inline-flex rounded-full bg-[#00b4b8] px-4 py-2 text-sm font-semibold text-white hover:bg-[#00a0a4]">Complete payroll setup</Link></div> : currentData ? <SummaryCards summary={currentData.summary} /> : null}
