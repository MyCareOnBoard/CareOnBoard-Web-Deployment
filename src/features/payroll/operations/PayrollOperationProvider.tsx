...
export function PayrollOperationProvider({ children }: { children: ReactNode }) {
  const watchers = useRef(new Map<string, { stopped: boolean; timer?: number; poll: () => Promise<PayrollOperation>; onTerminal?: () => void }>());
  ...
  const run = async (operationId: string) => { const current = watchers.current.get(operationId); if (!current || current.stopped || document.visibilityState === "hidden") return; try { const operation = await current.poll(); if (current.stopped || watchers.current.get(operationId) !== current || terminal.has(operation.state)) { if (terminal.has(operation.state)) settle(operationId, current); return; } current.timer = window.setTimeout(() => void run(operationId), operation.pollAfterMs ?? retryAfterMs); } catch { if (!current.stopped && watchers.current.get(operationId) === current) current.timer = window.setTimeout(() => void run(operationId), retryAfterMs); } };
  const watch = (scope: PayrollScope, operationId: string, poll: () => Promise<PayrollOperation>, onTerminal?: () => void) => {
    ...
    stop(key);
    watchers.current.set(key, { stopped: false, poll, onTerminal });
