...
export function PayrollOperationProvider({ children }: { children: ReactNode }) {
  const watchers = useRef(new Map<string, { stopped: boolean; timer?: number; inFlight: boolean; consecutiveErrors: number; poll: () => Promise<PayrollOperation>; onTerminal?: () => void }>());
  ...
  const run = async (operationId: string) => { const current = watchers.current.get(operationId); if (!current || current.stopped || current.inFlight || document.visibilityState === "hidden") return; current.inFlight = true; try { const operation = await current.poll(); if (current.stopped || watchers.current.get(operationId) !== current) return; current.inFlight = false; if (terminal.has(operation.state)) { settle(operationId, current); return; } current.consecutiveErrors = 0; schedule(operationId, current, operation.pollAfterMs ?? retryAfterMs); } catch { if (current.stopped || watchers.current.get(operationId) !== current) return; current.inFlight = false; current.consecutiveErrors += 1; schedule(operationId, current, Math.min(retryAfterMs * 2 ** (current.consecutiveErrors - 1), maxRetryAfterMs)); } };
