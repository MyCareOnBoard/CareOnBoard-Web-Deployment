...
describe("PayrollOperationProvider", () => {
  ...
  it("does not poll while the document is hidden and resumes when visible", async () => {
    vi.useFakeTimers(); let visibility = "hidden"; Object.defineProperty(document, "visibilityState", { configurable: true, get: () => visibility }); const poll = vi.fn().mockResolvedValue({ operationId: "op", state: "succeeded", resourceType: "company" });
  ...
  it("stops after a terminal backend state", async () => {
    vi.useFakeTimers(); const poll = vi.fn().mockResolvedValue({ operationId: "op", state: "succeeded", resourceType: "company" }); const settled = vi.fn();
  ...
  it("ignores a late terminal result after the same operation is replaced", async () => {
    ...
    await act(async () => { resolveFirst({ operationId: "op", state: "succeeded", resourceType: "company" }); });
