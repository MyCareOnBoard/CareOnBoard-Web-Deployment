...
export function PayrollApprovalDialog({
  ...
  const [loadDetail] = useLazyGetPayrollRunQuery();
  ...
  useEffect(() => {
    ...
    setDetail(null); setAcknowledged(false); setError(null); setLoading(true);
    const request = loadDetail({ ...scope, runId, activeRevisionId }, false) as AbortableRequest;
    ...
    return () => { if (requestRef.current === request) { request.abort?.(); requestRef.current = null; } };
  ...
  const submit = async () => {
    ...
    try {
      ...
      const typed = value as { code?: string; refreshRequired?: boolean; message?: string };
      setError(typed.message ?? "Payroll approval could not be started. Refresh and review the current payroll.");
      if (typed.refreshRequired || ["PROJECTION_STALE", "CURSOR_STALE", "CAPABILITY_DISABLED"].includes(typed.code ?? "")) onRefresh?.();
  ...
  return <Dialog open={open} onOpenChange={(next) => { if (!busy) onOpenChange(next); }}><DialogContent showCloseButton={!busy} className="max-h-[92vh] w-[min(95vw,44rem)] overflow-y-auto border border-[#dfe7e8] p-6" onCloseAutoFocus={(event) => { if (returnFocusRef?.current) { event.preventDefault(); returnFocusRef.current.focus(); } }}>
    <DialogHeader className="items-start gap-2 text-left"><DialogTitle className="text-xl leading-7">Approve payroll</DialogTitle><DialogDescription className="text-sm text-[#62686f]">Confirm the latest server-calculated preview before authorizing Check to process payroll.</DialogDescription></DialogHeader>
    {loading ? <p role="status" className="mt-5 text-sm text-[#62686f]">Loading current approval details…</p> : error && !detail ? <div className="mt-5"><p role="alert" className="text-sm text-[#9c3333]">{error}</p><button type="button" onClick={onRefresh} className="mt-3 min-h-11 rounded-lg border border-[#b8dfe0] px-4 text-sm font-semibold text-[#006f73]">Refresh payroll</button></div> : detail && totals ? <div className="mt-5 space-y-5">
